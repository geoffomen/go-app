...
func TestDatabase(t *testing.T) {
	...
	conf.Set("database.dialect", "sqlite")
	conf.Set("database.db", "/tmp/sqlite.db")
	defer func() {
		os.Remove("/tmp/sqlite.db")
	}()
	db, _ := NewGorm(GormConfig{
		Dialect: conf.GetStringOrDefault("database.dialect", "sqlite"),
		Db:      conf.GetStringOrDefault("database.db", "/tmp/sqlite.db"),
	})
