...
func logHandler() func(ctx *Ctx) error {
	return func(ctx *Ctx) error {
		...
		if err != nil {
			...
			var logMsg string
			args := ctx.getArgs()
			logMsg = fmt.Sprintf("status: %3d, method: %s, path: %s, cost: %13v, clientIp: %15s, args: %s, error: %s",
				statusCode,
				method,
				path,
				cost,
			...
			ctx.logger.Infof("status: %3d, method: %s, path: %s, cost: %13v, clientIp: %15s, args: %s, response: %s",
				http.StatusOK,
				method,
				path,
				cost,
				...
				reflectValueToString(ctx.getResponnse()),
			)
		}
